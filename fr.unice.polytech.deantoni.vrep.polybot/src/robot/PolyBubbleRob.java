...
public class PolyBubbleRob {
	...
	/**
	 * 
	 * @return true if an object is close detected. Detected object is store in {@link #handleDetectedObj} and sensor values in #detectedObjectPoint.
	 * A detected Surface Normal Vector is also store in {@link #mapDetectedObject}
	 */
	...
	/**
	 * make the robot go forward straight if @speed is positive and backward if @speed is negative
	 * @param speed: the desired speed (0 = stopped, negative = backward, positive = forward)
	 */
	 ...
	 * make the robot go forward straight if @speed is positive and backward if @speed is negative
	 * @param speed: the desired speed (0 = stopped, negative = backward, positive = forward)
	 * @param duration: the duration, in milliseconds during which the robot goes straight and then stop
	 */
	...
	public void goCurved(int speedLeft, int speedRight) {
		 vrep.simxSetJointTargetVelocity(clientID,leftMotor.getValue(),(float)speedLeft,remoteApi.simx_opmode_streaming);
		 vrep.simxSetJointTargetVelocity(clientID,rightMotor.getValue(),(float)speedRight,remoteApi.simx_opmode_streaming);
	}
